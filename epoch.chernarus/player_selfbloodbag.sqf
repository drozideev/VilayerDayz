...
if(_bloodbagTime < _bloodbagLastUsedTime) exitWith { // If cooldown is not done then exit script
        cutText [format["Вам надо отдышаться и придти в себя, прежде чем снова пытаться залить себе кровь. Подожди %1 секунд.",(_bloodbagTime - _bloodbagLastUsedTime)], "PLAIN DOWN"]; //display text at bottom center of screen when players cooldown is not done
};
 
if (_inCombat) then { // Check if in combat
    cutText [format["Вы не можете заливать себе кровь находясь в бою!"], "PLAIN DOWN"]; //display text at bottom center of screen when in combat
} else {
        ...
        if (_finished) then {
                ...
                if (random(_infectionChance) < 1) then {
                        ...
                        cutText [format["Вы успешно залили себе кровь, но увы, она была заражена."], "PLAIN DOWN"]; //display text at bottom center of screen if infected
                        ...
                        cutText [format["Вы успешно перелили себе кровь."], "PLAIN DOWN"]; //display text at bottom center of screen on succesful self bloodbag
                };
                ...
                cutText [format["Вы прекратили заливать себе кровь."], "PLAIN DOWN"]; //display text at bottom center of screen on interrupt
        };
};